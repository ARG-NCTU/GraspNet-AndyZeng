...
function DataLoader:getMiniBatch()
    ...
    for sampleIdx = 1,self.batchSize do
        batchColorPaths[sampleIdx] = paths.concat(self.dataPath,'color-input',self.samplePaths[self.shuffleIdx[self.trainIdx]])
        batchDepthPaths[sampleIdx] = paths.concat(self.dataPath,'depth-input',self.samplePaths[self.shuffleIdx[self.trainIdx]])
        batchLabelPaths[sampleIdx] = paths.concat(self.dataPath,'label',self.samplePaths[self.shuffleIdx[self.trainIdx]])
